...
1.	Click the **New** button in the bottom toolbar, and select **App Services | Active Directory | Directory | Custom Create**.
...
In this task, you will create a new MVC Application using **Visual Studio Express 2013 for Web** and will configure it to use **Organizational Accounts** as an Authentication method using the Active Directory tenant you created in the previous task.
...
1. In the **New ASP.NET Project - ExpenseReport** window, select the **MVC** template and then click **Change Authentication**.
	...
	_Selecting the MVC template and changing the authentication method_
...
1. In the **Change Authentication** dialog box, select **Organizational Accounts**, type the **Domain** created in the previous tasks (e. g. _yourorganization.onmicrosoft.com_) and click **OK**.
	...
	_Selecting Organizational Accounts as the Authentication method_
...
1. Switch to the **Configure** tab by clicking the **Configure** button at the top of the page.
...
1. The **single sign-on** section shows the required **APP ID URI** and **REPLY URL** properties which the service needs to drive the sign-in protocol flow.
	...
	The **REPLY URL** represents the address of your web application. Azure AD needs to know your application's address so that, after a user is successfully authenticated on Azure AD's pages, it can direct the flow back to your application.
...
1. In the **appSettings** section there are three new keys: **ida:FederationMetadataLocation**, **ida:Realm** and **ida:AudienceUri**. Those values were configured with the Federation Metadata Document and the App ID Uri of your application. The **IdentityConfig** class reads these values to configure the identity when the application starts.
...
This builds on the previous exercise and will show how to add capability to read directory data using the **Azure AD Graph API**. The Graph API is a new RESTful API that allows applications to access customers' data in Microsoft Azure directory.
...
1. In the **CONFIGURE** page, under the **permissions to other applications** section, select **Read directory data** in the **Application Permissions** drop-down list for the **Windows Azure Active Directory** application. 

	![Configure Application Permissions - Application Permissions](Images/configure-app-permissions---app-permissions.png?raw=true)

1. Also in the **CONFIGURE** page and under the **permissions to other applications** section, select **Read directory data** in the **Delegated Permissions** drop-down list. for the **Windows Azure Active Directory** application.
...
1. If not already open, start **Visual Studio 2013** and continue with the solution obtained in the previous exercise. Alternatively, you can open the **Begin.sln** solution from the **Source\Ex2-UsingGraphAPIWithWAAD\Begin** folder of this lab.
...
- [Securing a Windows Store Application and REST Web Service Using Azure AD (Preview)](http://aka.ms/t5ejfa): This document will show you how to create a simple web API resource and a Windows Store client application using the Microsoft Azure Authentication Library and Azure AD.
...
1. In the Download Manifest dialog box, click **Download Manifest**. When prompted, save the file and close the dialog box.

	![download-manifest-dialog](Images/download-manifest-dialog.png?raw=true "Download Manifest")
	
	_Download Manifest_
	
1. Open the application manifest file with **Visual Studio**. Find the **requiredResourceAccess** section and add the section highlighted below. Save the file.
...
1. Wait until the operation is complete and the page refreshes.
...
1. Notice that in the **Permissions to other applications** section, both permissions have been updated.
