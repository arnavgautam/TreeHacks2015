...
TicTacToeController.prototype.finish = function () {

    var color = this.game.nextColor(this.viewModel.playerColor());
    var action = { type: 2, commandData: { color: color} };
    var gameId = this.viewModel.gameId();

    if (this.gameService.status == 2 && !this.game.hasWinner() && !this.game.isTie()) {
        this.gameService.sendGameAction(gameId, action);
        alert("You are leaving the active game");
    }
};
...
TicTacToeController.prototype.processAction = function (gameAction) {
    ...
    if (gameAction.Type == 2) {
        this.game.setWinner(gameAction.CommandData.color);
        if (this.finished) {
            return;
        }
        else {
