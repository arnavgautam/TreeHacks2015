...
GameService.prototype.sendGameAction = function (gameId, action, success, error) {
    this.serverInterface.sendAjaxJsonPost(this.apiURL + "api/game/command/" + gameId, JSON.stringify(action), success, error);
};

GameService.prototype.postStatisticsEvent = function (action, success, error) {
    this.serverInterface.sendAjaxJsonPost(this.apiURL + "api/event/PostEvent/statistics", JSON.stringify(action), success, error);
};
